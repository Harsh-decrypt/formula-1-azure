...
results_df = spark.read.format("delta").load(f"{processed_folder_path}/results").withColumnRenamed("time", "race_time") \
...
final_df = race_results_df.select("race_id", "race_year", "race_name", "race_date", "circuit_location", "driver_name", "driver_number", "driver_nationality",
                                 "team", "grid", "fastest_lap", "race_time", "points", "position", "result_file_date").withColumn("created_date", current_timestamp()) \
